...
def compute_results(actual_labels,pred_labels):
    ...
    for i in range(14):
        precision[i] = round(float(precision[i]),3)
        recall[i] = round(float(recall[i]),3)
        f1[i] = round(float(f1[i]),3)
...
if __name__ == '__main__':
    ...
    accuracy_table = []
    for i in range(11):
        accuracy,pred = MNB.predict(x_test, y_test,i/10)
        accuracy_table.append(accuracy)
        compute_results(y_test,pred)
        print("Accuracy {0:.4f}".format(accuracy))

    for i in range(11):
        print('accuracy for case lambda_mix = ',i/10,':',accuracy_table[i])
